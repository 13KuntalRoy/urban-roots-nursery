...
from unfold.admin import ModelAdmin  # ← add this import
from .models import SiteConfig, Tree, Feature, OrderStep

@admin.register(SiteConfig)
class SiteConfigAdmin(ModelAdmin):  # ← changed
...
@admin.register(Tree)
class TreeAdmin(ModelAdmin):  # ← changed
    list_display = ('name', 'category', 'price', 'created_at')
    list_filter = ('category',)
    search_fields = ('name', 'description')

@admin.register(Feature)
class FeatureAdmin(ModelAdmin):  # ← changed
    list_display = ('title', 'icon_name', 'order')
    list_editable = ('order',)

@admin.register(OrderStep)
class OrderStepAdmin(ModelAdmin):  # ← changed
